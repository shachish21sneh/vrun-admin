...
<div>
        ...
        <div className="text-right">
          <p className="font-mono text-sm">{car.registration_number}</p>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
        ...
        <span>
          🗓️{" "}
          {car.plan_start
            ? `${new Date(car.plan_start).toLocaleDateString()} → ${new Date(
                car.plan_end!,
              ).toLocaleDateString()}`
            : "No plan period"}
        </span>
      ...
      <div className="text-xs text-muted-foreground">
        Created: {new Date(car.created_at).toLocaleString()}
      </div>
    </div>
  ))}
</div>
